resource "null_resource" "phonebook_cluster_bastion_setup" {
  ...
  provisioner "file" {
    ...
    destination = "${local.homeDir}/.ssh/id_rsa"
  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "cd ${local.homeDir}",
      "chown ${local.user}:${local.user} ./.ssh/id_rsa",
      "chmod og-rwx ./.ssh/id_rsa",
      "ssh -o StrictHostKeyChecking=no ${local.user}@${aws_instance.phonebook_cluster_workernode1.private_ip} \"sudo cloud-init status --wait\"",
      "scp -o StrictHostKeyChecking=no ${local.user}@${aws_instance.phonebook_cluster_workernode1.private_ip}:/etc/rancher/k3s/k3s.yaml .",
      "mkdir -p ./.kube",
