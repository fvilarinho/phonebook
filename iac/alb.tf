resource "aws_lb" "phonebook_cluster" {
  name                       = "${var.settings.general.name}-cluster-lb"
...
resource "aws_lb_target_group" "phonebook_cluster" {
  name        = "${var.settings.general.name}-cluster-lb-tg"
  port        = 80
  protocol    = "HTTP"
...
resource "aws_lb_listener" "secure_phonebook_cluster" {
  ...
  port              = 443
  protocol          = "HTTPS"
  certificate_arn   = aws_acm_certificate_validation.phonebook_cluster.certificate_arn
  ...
  depends_on = [
    ...
    aws_acm_certificate_validation.phonebook_cluster,
    aws_lb_target_group.phonebook_cluster
  ]
}
